...
int main(string[] args)
{
			...
			foreach (line; file.byLine()) {
				...
				if (!c.empty) {
					...
					if (p) {
						...
						if ((key == "remote_dir") && (c.front.dup != cfg.getValueString("remote_dir"))) {
							log.vdebug(key, " was modified since the last time the application was successfully run, --resync needed");
							configOptionsDifferent = true;
						}
	...
	if (cfg.getValueBool("display_config")){
		...
		writeln("Config option 'remote_dir'            = ", cfg.getValueBool("remote_dir"));
	...
	if ((cfg.getValueBool("synchronize")) || (cfg.getValueBool("resync")) || (cfg.getValueBool("monitor"))) {
		...
		if ((cfg.getValueBool("synchronize")) || (cfg.getValueBool("resync"))) {
			if (online) {
				...
				performSync(sync, cfg.getValueString("single_directory"), cfg.getValueString("remote_dir"), cfg.getValueBool("download_only"), cfg.getValueBool("local_first"), cfg.getValueBool("upload_only"), LOG_NORMAL, true);
			}
		}
			...
			while (true) {
				...
				if (currTime - lastCheckTime > checkInterval) {
					...
					try {
						...
						try {
							performSync(sync, cfg.getValueString("single_directory"), cfg.getValueString("remote_dir"), cfg.getValueBool("download_only"), cfg.getValueBool("local_first"), cfg.getValueBool("upload_only"), (logMonitorCounter == logInterval ? MONITOR_LOG_QUIET : MONITOR_LOG_SILENT), fullScanRequired);
...
void performSync(SyncEngine sync, string singleDirectory, string remotePath, bool downloadOnly, bool localFirst, bool uploadOnly, long logLevel, bool fullScanRequired)
{
	int count;
	// string remotePath = "/";
    string localPath = ".";
